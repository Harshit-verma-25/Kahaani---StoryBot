...
const StoryPageClient = ({ slug }: StoryPageClientProps) => {
  ...
  const output = (storyInfo?.[language] as inBuiltStoryVideoAudio) || undefined;
  const videoUrl = storyInfo?.videoUrl ?? undefined;
  const images = storyInfo?.images ?? [undefined];

  const availableLanguages = LANGUAGE_OPTIONS.filter(
    (option) => storyInfo?.[option.value]?.story,
  );

  useEffect(() => {
    if (!output?.story && videoUrl) {
      setActiveTab("Full Video");
    }
  }, [output?.story, videoUrl]);
    ...
    <section className="min-h-screen bg-background flex items-center justify-center ">
      <div className="max-w-7xl w-full mx-auto flex flex-col items-center justify-center gap-6 px-4">
        ...
        <div className="bg-transparent w-full flex flex-col items-center justify-center">
          ...
          {activeTab === "Full Story" && output?.story && (
